#' ---
#' title: Simulations for spatial confounding in joint species distribution models -- Negative binomial responses where the observed covariates are now fine scaled, compared to the residual component of the unobserved covariate.
...
here::i_am("simulation/negbin_N400/sims_negbin_finescaleX.R")
library(here)
...
true_dispparam <- runif(num_spp, 0, 5)
...
simfn <- function(NAI, scale_X = 0) {
    ...
    X1 <- (big_basis_fn[,c(11:14)] %*% rnorm(4, sd = 0.25))
    cov_dat$climate <- 0.5*X1 + rnorm(num_sites, sd = 0.05)
    
    eps1 <- big_basis_fn[,c(2,4,6)] %*% rnorm(3, sd = 0.1)
    eps2 <- big_basis_fn[,c(3,5,7)] %*% rnorm(3, sd = 0.1)
    ...
    save(X_coefficients_dat, 
         ...
         LV_results_dat, 
         LVcor_results_dat,
         file = paste0("simulation_negbin_finescaleX_N", num_sites, "_scaleX", scale_X, "_dataset", NAI, ".RData"))
    }
...
for(k0 in 1:length(scale_X_seq)) { for(k1 in 1:num_datasets) {
    make_filename <- here("simulation", "negbin_N400", paste0("simulation_negbin_finescaleX_N", num_sites, "_scaleX", scale_X_seq[k0], "_dataset", k1, ".RData"))
