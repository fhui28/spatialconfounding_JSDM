...
here::i_am("simulation/negbin_N400/sims_negbin.R")
library(here)
...
simfn <- function(NAI, scale_X = 0) {
    ...
    save(X_coefficients_dat, 
         ...
         LV_results_dat, 
         LVcor_results_dat,
         file = paste0("simulation_negbin_N", num_sites, "_scaleX", scale_X, "_dataset", NAI, ".RData"))
    }
...
for(k0 in 1:length(scale_X_seq)) { for(k1 in 1:num_datasets) {
    make_filename <- here("simulation", "negbin_N400", paste0("simulation_negbin_N", num_sites, "_scaleX", scale_X_seq[k0], "_dataset", k1, ".RData"))
